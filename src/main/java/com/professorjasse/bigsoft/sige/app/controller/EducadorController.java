...
//    @Get
    ...
    public void entrar(Educador entity) {
        for (Educador educa : dao.findAllUsers()) {
            if (educa.getNome().equals(entity.getNome())) {
//                JOptionPane.showMessageDialog(null, "logged");
                result.redirectTo(EducadorController.class).avaliacaoPeriodica();
    ...
    public void add(Educador entity, String nome) {
        ...
        entity.setNome(nome);
        dao.create(entity);
        result.redirectTo(EducadorController.class).create();
    }
...
//        result.include("educador", idEducadorLogado);
    ...
    @Get
    @Path("/avaliacaoPeriodicaAlunoDetalhes")
    public void avPeriodicaAlunoDetalhes(int id) {
        result.include("avaliacao", avaliacaoPeriodicaDao .find(id));
        result.include("aluno", avaliacaoPorAreaDAO.find(id).getDoente());
    }
    ...
    public void addAvaliacaoPorArea(AvaliacaoPorArea entity) {
        ...
        result.redirectTo(EducadorController.class).avaliacaoPorArea();
    }
